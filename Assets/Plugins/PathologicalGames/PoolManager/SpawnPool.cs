    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        public void CreatePrefabPool(PrefabPool prefabPool)
        {
            ...
            if (!isAlreadyPool)
            {
                ...
                this.prefabs._Add(prefabPool.name, prefabPool.prefab);
            }
        ...
        public void RemovePrefabPool(string name) {
            for (int i = 0; i < _prefabPools.Count; ++i) {
                ...
                if (prefabPool.name.Equals(name)) {
                    for (int j = 0; j < prefabPool._despawned.Count; ++j) {
                        if (prefabPool._despawned[j] != null)
                            GameObject.Destroy(prefabPool._despawned[j].gameObject);    
                    }
                    for (int j = 0; j < prefabPool._spawned.Count; ++j) {
                        if (prefabPool._spawned[j] != null)
                            GameObject.Destroy(prefabPool._spawned[j].gameObject);    
                    }
                    prefabs._Remove(prefabPool.name);
        ...
        /// just before despawn.
        /// </summary>
        public void Despawn(Transform instance, Transform parent, string instName = "")
        {
            //Trace.XHJ("将" + instName + "放入对象池中~");
            //if (instance == null) 
            //    return;
            if (instance) {
                if (parent)
                ...
                Debug.LogWarning(instName + "未放入对象池中");
            }
        }
        ...
        private IEnumerator DoDespawnAfterSeconds(Transform instance, float seconds, bool useParent, Transform parent)
        {
            ...
            if (seconds > 0 && !IsSpawned(instance)) 
                yield break;
            ...
            if (instance) {
                if (useParent) {
                    if (parent)
                        this.Despawn(instance, parent, instName);
                } else {
                    this.Despawn(instance);
                }
            }
